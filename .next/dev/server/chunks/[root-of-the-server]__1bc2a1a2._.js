...
"[project]/fantasy-web/app/api/nba-stats/route.ts [app-route] (ecmascript)", ((__turbopack_context__) => {
"use strict";
...
function calculateFantasyPoints(stats) {
    return (stats.pts || 0) * FANTASY_WEIGHTS.pts + (stats.reb || 0) * FANTASY_WEIGHTS.reb + (stats.ast || 0) * FANTASY_WEIGHTS.ast + (stats.stl || 0) * FANTASY_WEIGHTS.stl + (stats.blk || 0) * FANTASY_WEIGHTS.blk + (stats.fg3m || 0) * FANTASY_WEIGHTS.fg3m + (stats.tov || 0) * FANTASY_WEIGHTS.tov;
}
// 后台刷新数据
async function refreshData() {
    ...
    try {
        ...
        for (const date of recentDates){
            ...
            const finishedGames = (gamesRes.data || []).filter((g)=>g.status === "Final");
        ...
        let injuries = [];
        try {
            const injuriesRes = await fetchAPI("/player_injuries", {
                per_page: "100"
            });
...
//# sourceMappingURL=%5Broot-of-the-server%5D__1bc2a1a2._.js.map